...
import { AuthCache,retry } from '@xmtp/xmtp-js/node'
import type {
  ApiClient,
  Authenticator,
  NetworkOptions,
...
import { MlsApiClient } from "./gen/mls/api/v1/mls.client.js"

const API_URLS: { [k: string]: string } = {
  dev: 'https://dev.xmtp.network:5555',
  production: 'https://production.xmtp.network:5555',
}
...
export default class GrpcApiClient implements ApiClient {
  ...
  constructor(apiUrl: string, isSecure: boolean, appVersion?: string) {
    ...
    this.grpcClient = new MessageApiClient(new GrpcWebFetchTransport({ baseUrl: apiUrl }))
    this.mlsClient = new MlsApiClient(new GrpcWebFetchTransport({ baseUrl: apiUrl }))
  }
